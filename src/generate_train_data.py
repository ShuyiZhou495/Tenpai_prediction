...
def generate_train_data(file_name):
    ...
    try:
        return np.load(f"./data/{file_name}_x.npy", allow_pickle=True), np.load(f"./data/{file_name}_y.npy")
    except:
        ...
        np.save(f"./data/{file_name}_y.npy", y_data_numpy)
        np.save(f"./data/{file_name}_x.npy", x_data_numpy)
        ...
        for each_inp in inp:
            ...
            for action in each_inp.split(" "):
                if action != "":
                    if not(player != action[0] and action[1] == "G"):
                        x.append(action_to_vector(action, player, chanfon, jikaze, dora_list))
            x_data.append(np.array(x, dtype=np.bool))
            y_data.append(tenpai_result)
...
def pad_x(x_data):
    ...
    max_x_len = max(x_len)
    np.save(local_place + "x_len.npy", np.array(x_len, dtype=np.int8))
    # x_data_ret = np.zeros((len(x_data), max_x_len, 52), dtype=bool)
    x_data_ret = np.load(local_place + "x_data.npy", allow_pickle=True)

    for i in range(700000,len(x_data)):
        x_data_ret[i, -len(x_data[i]):] = x_data[i].astype(bool)
        if i%10000==0:
            print(i, end="\r")
            np.save(local_place + "x_data.npy", x_data_ret)
    return x_data_ret


def shuffle_split(x_data, x_len, y_data):
    # x_data, y_data = shuffle(x_data, y_data)
    return train_test_split(x_data, x_len, y_data, test_size=0.2)
...
def generate_train_test_local():
    x_data = np.load(local_place + "x_data.npy").astype(bool)
    y_data = np.load(local_place + "y_data.npy").astype(bool)
    x_len = np.load(local_place + "x_len.npy").astype(np.int8)
    
    return shuffle_split(x_data, x_len, y_data)
